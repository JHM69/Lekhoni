...
class j {
  ...
  constructor({ api: a, config: i, onSelectFile: c, readOnly: r }) {
    this.api = a, this.config = i, this.onSelectFile = c, this.readOnly = r, this.nodes = {
      ...
      categoriesPreloader: M("div", this.CSS.categoriesPreloader),
      categories: M("div", [this.CSS.input, this.CSS.categories], {
        contentEditable: !this.readOnly
    })
    }, this.nodes.caption.dataset.placeholder = this.config.captionPlaceholder, this.nodes.categories.dataset.placeholder = this.config.categoriesPlaceholder, this.nodes.imageContainer.appendChild(this.nodes.imagePreloader), this.nodes.wrapper.appendChild(this.nodes.imageContainer), this.nodes.wrapper.appendChild(this.nodes.caption), this.nodes.wrapper.appendChild(this.nodes.categories), this.nodes.wrapper.appendChild(this.nodes.fileButton);
  }
  /**
   * CSS classes
   *
  ...
  get CSS() {
    return {
      ...
      categories: "image-tool__categories",
    };
  }
  /**
   * Ui statuses:
  ...
  fillCaption(a) {
   ...
   /**
   * Shows categories input
   *
   * @param {string} text - categories text
   * @returns {void}
   */
  fillCategories(a) {
    this.nodes.categories && (this.nodes.categories.innerHTML = a);
  }
  /**
   * Changes UI status
   *
...
class P {
  ...
  constructor({ data: a, config: i, api: c, readOnly: r, block: n }) {
    this.api = c, this.readOnly = r, this.block = n, this.config = {
      ...
      categoriesPlaceholder : this.api.i18n.t(i.categoriesPlaceholder || "Categories"),
   ...
   */
  set data(a) {
    this.image = a.file, this._data.caption = a.caption || "", this._data.categories = a.categories || "", this.ui.fillCaption(this._data.caption), this.ui.fillCategories(this._data.categories), P.tunes.forEach(({ name: i }) => {
  ...
  onUpload(a) {
    if(a.success && a.file) { 
      this.image = a.file;
      this._data.categories = a.categories;
      this.ui.fillCategories(this._data.categories);  
    } else {
      this.uploadingFailed("incorrect response: " + JSON.stringify(a));
