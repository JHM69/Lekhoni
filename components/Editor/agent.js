...
export default class MentionTool {
    ...
    createSearchbar(accessKey, allUsers) {
        ...
        const searchTextbox = create('input', [this.CSS.searchTextbox], {
            type: 'text',
            placeholder: accessKey === "@" ? "Search for Agents" : "Search for Tags"
        });
        ...
        async function searchQueryListner() {
            if (classObj.searchAPIUrl) {
                try {
                    ...
                    await new Promise(resolve => setTimeout(resolve, 600)); // Delay of 1000ms (1 second)

                    try {
                        const response = await fetch(classObj.searchAPIUrl + searchQuery, { signal });
                        if (response.ok) {
                            await response.json().then(data => {
                                /**
                                * Creates user list items from the received user objects.
                                */
                                const userListItems = classObj.createAllUserListItems(data.users, '', accessKey);
                                const newChild = classObj.createUsersList(userListItems);
                                if (classObj.nodes.userMentionToolbar.lastChild) {
                                    classObj.nodes.userMentionToolbar.removeChild(classObj.nodes.userMentionToolbar.lastChild);
                                }

                                // Now, add the new child as the last child of userMentionToolbar
                                classObj.nodes.userMentionToolbar.appendChild(newChild);
                        ...
                        // Reset abortController to null after the request completes or is aborted
    ...
    createUserListItem({ userId, userFullName, userAvatar, userSlug, baseUrl, accessKey }) {
        ...
        userListItemWrapper.addEventListener('click', function () {
            ...
            // Send a custom event on google analytics
            ReactGA.event({
                category: 'agent',
                action: userSlug + " agent called",
                label: userSlug, // optional
